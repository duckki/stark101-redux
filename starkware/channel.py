...
class Channel(object):
    ...
    def __init__(self):
        self.state = sha256('0'.encode()).hexdigest()
        self.proof = []

    def send(self, tag, s):
        self.state = sha256((self.state + tag + str(s)).encode()).hexdigest()
        self.proof.append((inspect.stack()[0][3], tag, s))
    ...
    def receive_random_int(self, min, max, show_in_proof=True):
        ...
        if show_in_proof:
            self.proof.append((inspect.stack()[0][3], num))
        return num
